...
class Progress_pekerjaan extends CI_Controller
{
    ...
    if (!$row){
      ...
      if ($row){
        $data = array(
          ...
          'progress' => $this->Progress_pekerjaan_model->get_progress(),
          'progress_option' => $this->Progress_pekerjaan_model->get_progress_option(),
  ...
  public function create_action(){
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      //CEK APAKAH PEKERJAAN DIBOLEHKAN MEMILIKI MULTI KONTRAK
      $row = $this->Pekerjaan_model->get_by_id($this->input->post('id_p',TRUE));
      if ($row->multi_kontrak == 'ya'){
        //YA SUM NILAI KONTRAK
        $total_kontrak = $this->Kontrak_model->sum_nilai_kontrak($this->input->post('id_p',TRUE));        

      }else{
        //TIDAK AMBIL YG PALING TINGGI
