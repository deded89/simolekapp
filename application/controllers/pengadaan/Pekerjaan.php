...
class Pekerjaan extends CI_Controller
{
  ...
  public function read($id)
  {
    ...
    if (!$row) {
      ...
      $persen_real_keu = $this->Progress_pekerjaan_model->get_persen_real_keu($id, $row->multi_kontrak);
      ...
      if ($row) {
        $data = array(
          ...
          'multi_kontrak' => $row->multi_kontrak,
  ...
  public function create_action()
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      $data = array(
        ...
        'multi_kontrak' => $this->input->post('multi_kontrak', TRUE),
      );
  ...
  public function update($id)
  {
    ...
    if ($row) {
      $data = array(
        ...
        'tahun_anggaran' => set_value('pagu', $row->tahun_anggaran),
        'multi_kontrak' => $row->multi_kontrak,
      );
      $this->load->view('template_view', $data);
    } else {
  ...
  public function cetak()
  {
    ...
    foreach ($pekerjaan_data as $pekerjaan) {
      ...
      $sheet->setCellValue('K' . $i, $pekerjaan->username);
    ...
    if ($this->session->userdata('tahun_anggaran') == '2020') {
      ...
      $sheet->setCellValue('C' . $hr, 'NAMA SUB KEGIATAN');
    }
    ...
    $sheet->setCellValue('K' . $hr, 'USERNAME');
  ...
  private function generate_qr($id_p)
  {
    ...
    $config['imagedir']     = './assets/temp/'; //direktori penyimpanan qr code
    ...
    $image_name = $id_p . '_detail_p.png'; //buat name dari qr code sesuai dengan nim

    $params['data'] = site_url('pengadaan/pekerjaan/read/' . $id_p); //data yang akan di jadikan QR CODE
  ...
  public function cetak_detail($id_p)
  {
    ...
    $persen_real_keu = $this->Progress_pekerjaan_model->get_persen_real_keu($id_p, $pekerjaan_data->multi_kontrak);
    ...
    if ($this->session->userdata('tahun_anggaran') == '2020') {
      ...
      $sheet->setCellValue('B' . ($sr + 4), 'Nama Sub Kegiatan');
    }
    ...
    ///GENERATE QR
    $this->generate_qr($id_p);

    $drawing = new \PhpOffice\PhpSpreadsheet\Worksheet\HeaderFooterDrawing();
    $drawing->setName('PhpSpreadsheet logo');
    $drawing->setPath('./assets/temp/' . $id_p . '_detail_p.png');
    $drawing->setWidth(70);
    $sheet->getHeaderFooter()->addImage($drawing, \PhpOffice\PhpSpreadsheet\Worksheet\HeaderFooter::IMAGE_FOOTER_RIGHT);
    $sheet->getHeaderFooter()->setOddFooter('&R&G');
    ...
    $writer = new Xlsx($spreadsheet);
    $writer->save('php://output');
    unlink(FCPATH . 'assets/temp/' . $id_p . '_detail_p.png');
  }
}
