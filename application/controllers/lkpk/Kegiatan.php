...
class Kegiatan extends CI_Controller
{
  function __construct()
  {
    ...
    $this->load->model('lkpk/Ren_real_model');
    $this->load->model('Skpd_model');
    $this->load->library('form_validation');
    if (!$this->ion_auth->logged_in())
    {
      redirect('auth/login', 'refresh');
    }else if ($this->permission->permission_check('akses_kegiatan') == false) {
      return show_error('Hak Akses Anda Tidak Mencukupi.');
    }
  }

  public function index()
  {
    if ($this->permission->permission_check('view_kegiatan') == false){
      return show_error('Hak Akses Anda Tidak Mencukupi.');
    }
    if ($this->permission->permission_check('per_skpd') == false){
      $kegiatan = $this->Kegiatan_model->get_all();
    }else {
      $kegiatan = $this->Kegiatan_model->get_per_skpd();
    }
  ...
  public function skpd($id_skpd)
  {
    if ($this->permission->permission_check('view_kegiatan') == false){
      return show_error('Hak Akses Anda Tidak Mencukupi.');
    }

    $kegiatan = $this->Kegiatan_model->get_by_id_skpd($id_skpd);
  ...
  public function ubah_pengecualian($i, $id_k){
    if ($i == 0){
      $this->Kegiatan_model->ubah_ke_yes($id_k);
    } else {
      $this->Kegiatan_model->ubah_ke_no($id_k);
    }
    redirect(site_url('lkpk/kegiatan'));
  }
...
//#######################  DATA REALISASI
  ...
  public function download_format_realisasi(){
    ...
    $sheet->setCellValue('U'.($sr+0),'Januari');
    $sheet->setCellValue('V'.($sr+0),'Februari');
    $sheet->setCellValue('W'.($sr+0),'Maret');
    $sheet->setCellValue('X'.($sr+0),'April');
    $sheet->setCellValue('Y'.($sr+0),'Mei');
    $sheet->setCellValue('Z'.($sr+0),'Juni');
    $sheet->setCellValue('AA'.($sr+0),'Juli');
    $sheet->setCellValue('AB'.($sr+0),'Agustus');
    $sheet->setCellValue('AC'.($sr+0),'September');
    $sheet->setCellValue('AD'.($sr+0),'Oktober');
    $sheet->setCellValue('AE'.($sr+0),'Nopember');
    $sheet->setCellValue('AF'.($sr+0),'Desember');
    ...
    if($data_kegiatan){
      ...
      foreach ($data_kegiatan as $kegiatan){
        ...
        if ($data_realisasi) {
          $sheet->setCellValue('U'.($sr+$i),$data_realisasi->kumb01);
          $sheet->setCellValue('V'.($sr+$i),$data_realisasi->kumb02);
          $sheet->setCellValue('W'.($sr+$i),$data_realisasi->kumb03);
          $sheet->setCellValue('X'.($sr+$i),$data_realisasi->kumb04);
          $sheet->setCellValue('Y'.($sr+$i),$data_realisasi->kumb05);
          $sheet->setCellValue('Z'.($sr+$i),$data_realisasi->kumb06);
          $sheet->setCellValue('AA'.($sr+$i),$data_realisasi->kumb07);
          $sheet->setCellValue('AB'.($sr+$i),$data_realisasi->kumb08);
          $sheet->setCellValue('AC'.($sr+$i),$data_realisasi->kumb09);
          $sheet->setCellValue('AD'.($sr+$i),$data_realisasi->kumb10);
          $sheet->setCellValue('AE'.($sr+$i),$data_realisasi->kumb11);
          $sheet->setCellValue('AF'.($sr+$i),$data_realisasi->kumb12);
        } else {
          for ($x = 0; $x < 12; $x++) {
            $sheet->setCellValue($range_kum[$x].($sr+$i),0);
          }
        }
        $x=1;
        $bulan_column2 = array('J','K','L','M','N','O','P','Q','R','S','T');
        foreach ($bulan_column2 as $col) {
          $sheet->setCellValue(''.$col.($sr+$i).'','='.$range_kum[$x].($sr+$i).'-'.$range_kum[$x-1].($sr+$i));
          $x++;
        }
        $sheet->setCellValue('I'.($sr+$i),'=U'.($sr+$i));
      ...
      $sheet->getStyle($range_kum[$bulan_realisasi].($sr+2).':'.$range_kum[$bulan_realisasi].($sr+$i-1))->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('89BAEB');
      $sheet->getStyle('C'.($sr+1).':F'.($sr+$i-1))->getNumberFormat()->setFormatCode(('#,##0.00'));
      $sheet->getStyle('I'.($sr+1).':AF'.($sr+$i-1))->getNumberFormat()->setFormatCode(('#,##0.00'));
      $sheet->getStyle('A'.($sr+1).':AF'.($sr+$i-1))->applyFromArray($style_table_data);
    } else {
      $this->session->set_flashdata('error',"Kegiatan Tidak Ditemukan");
      redirect(site_url('lkpk/kegiatan'));
