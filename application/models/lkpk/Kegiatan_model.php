...
class Kegiatan_model extends CI_Model
{
  ...
  function get_all()
  {
    ...
    if ($this->permission->permission_check('per_skpd')==true){
  ...
  // get all
  function get_by_id_skpd($id_skpd)
  {
    $this->db->select('k.*,s.*');
    $this->db->from('epiz_21636198_lkpk.kegiatan k');
    $this->db->join('epiz_21636198_simolek.skpd s', 's.id_skpd=k.skpd', 'left');
    $this->db->where('s.id_skpd', $id_skpd);
    $this->db->order_by($this->id, $this->order);
    return $this->db->get()->result();
  }
  // get all
  function get_per_skpd()
  {
    $this->db->select('k.*,s.*');
    $this->db->from('epiz_21636198_lkpk.kegiatan k');
    $this->db->join('epiz_21636198_simolek.skpd s', 's.id_skpd=k.skpd', 'left');
  ...
  function ubah_ke_no($id_k){
    $this->db3->set('insidentil',0);
    $this->db3->where('id_kegiatan', $id_k);
    $this->db3->update('kegiatan');
  }

  function ubah_ke_yes($id_k){
    $this->db3->set('insidentil',1);
    $this->db3->where('id_kegiatan', $id_k);
    $this->db3->update('kegiatan');
  }
