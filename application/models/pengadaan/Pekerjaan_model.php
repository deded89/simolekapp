...
class Pekerjaan_model extends CI_Model
{
  ...
  function get_all()
  {
    ...
    $this->db->where('p.tahun_anggaran', $this->session->userdata('tahun_anggaran'));
  ...
  {
    $this->db->select('p.tahun_anggaran, p.id as id_p, p.deskripsi, p.id_rup, p.id_lpse, p.link_rup, p.link_lpse, p.nama, p.kegiatan, p.progress_now, pr.id, pr.nama as pr_now, s.id_skpd, s.nama_skpd, j.id as id_j, j.nama as jenis, m.id as id_m, m.nama as metode, p.pagu, p.multi_kontrak');
    $this->db->from('epiz_21636198_pengendalian.pekerjaan p');
  ...
  function update_id_pengadaan($id, $data)
  {
    ...
    $this->db2->set('link_rup', 'https://sirup.lkpp.go.id/sirup/ro/cari?tahunAnggaran=' . $data['tahun_anggaran'] . '&keyword=' . $data['id_rup'] . '&jenisPengadaan=0&metodePengadaan=0');
  ...
  function get_proyeksi($id_p)
  {
    ...
    if ($q_data) {
      ...
      if ($hari_terlaksana->format('%a') + 1 > $masa_kontrak->format('%a') + 1) {
        $hari_terlaksana = $masa_kontrak;
      }

      $persen_perday = 100 / ($masa_kontrak->format('%a') + 1);
      $proyeksi_today = $persen_perday * ($hari_terlaksana->format('%a') + 1);

      if ($proyeksi_today > 100) {
        $proyeksi_today = 100;
      }
      return array(
  ...
  function cetak()
  {
    ...
    $db2->select('p.*,p.nama as nama_pekerjaan, pr.nama as progress, s.*, j.nama as jenis, m.nama as metode, pic.*, pic.nama as nama_pic, u.username');
