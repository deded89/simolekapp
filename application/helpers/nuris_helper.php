...
function cmb_pekerjaan_list($name,$table,$field1,$field2,$pk,$selected){
    $ci = get_instance();
    $cmb = "<select name='$name' id='$name' class='form-control select2' style='width: 100%'>";
    $ci->db->where('tahun_anggaran',$ci->session->userdata('tahun_anggaran'));
    $data = $ci->db->get($table)->result();
	$cmb .= "<option></option>";
    foreach ($data as $d){
        $cmb .="<option value='".$d->$pk."'";
        $cmb .= $selected==$d->$pk?" selected='selected'":'';
        $cmb .=">".($d->$field1)."- Tahun ".($d->$field2)."</option>";
    }
    $cmb .="</select>";
    return $cmb;
}
